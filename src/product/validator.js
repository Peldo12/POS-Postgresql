...
const createProductSchema = joi
  .object({
    sku: joi.string().alphanum().required().label('SKU'),
    barcode: joi.string().alphanum().min(3).required().label('Barcode'),
    name: joi.string().required().label('Name'),
    buy: joi.number().positive().required().label('Buy'),
    sell: joi.number().positive().required().label('Sell'),
    category_id: joi.number().integer().min(1).required().label('Category ID'),
    stock: joi
      .number()
      .integer()
      .positive()
...
const updateProductSchema = joi
  .object({
    ...
    category_id: joi.number().integer().min(1).label('Category ID'),
    stock: joi.number().integer().default(0).optional().label('Stock'),
    minimum_stock: joi.number().integer().optional().label('Minimum stock'),
...
const updateStockSchema = joi
  .object({
    transactions: joi
      .array()
      .items({
        id: joi.number().integer().required().label('ID'),
        updateStock: joi.number().integer().required().label('Update Stock'),
      })
      .required()
      .label('Transactions'),
  })
  .required()
  .label('Update Stock Schema');
...
const filterSchema = joi
  .object({
    ...
    search: joi.string().allow('').optional().label('Seacrh'),
    minPrice: joi.number().min(0).optional().label('Minimum Price'),
    maxPrice: joi.number().min(0).optional().label('Max Price'),
    categoryId: joi.number().integer().optional().label('Category ID'),
    sort: joi
      .string()
      .valid('id', 'sku', 'name', 'sell', 'stock')
      .optional()
      .label('Sort'),
    orderBy: joi.string().valid('ASC', 'DESC').optional().label('Order By'),
  })
  .optional()
  .label('Filter Schema');
